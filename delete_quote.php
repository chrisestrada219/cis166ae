...
if (!is_administrator()) {
	print '<h2>Access Denied!</h2><p class="error">You do not have 
...
if (isset($_GET['id']) && 
is_numeric($_GET['id']) && 
($_GET['id'] > 0) ) { // Display the quote in a form:

	// Define the query.
	$query = "SELECT quote, source, favorite FROM myquotes WHERE id={$_GET['id']}";
	
	if ($result = mysqli_query($dbc, $query)) { // Run the query.

		$row = mysqli_fetch_array($result); // Retreive the info.
...
// Handle the form.
	...
	$resutlt = mysqli_query($dbc, $query); // Execute the query.
